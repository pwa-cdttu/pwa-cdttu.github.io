    ...
    transform(value: any, searchText: any): any {
        ...
        return value?.filter((ch: any) => {
            return this.commonService.generatedSlug(`${searchText}`)?.split('-')
                .every((x) => {
                    return this.commonService.generatedSlug(`${ch.id}`)?.includes(x) || this.commonService.generatedSlug(`${ch.na}`)?.includes(x) || this.commonService.generatedSlug(`${ch.bi}`)?.includes(x)
                })
        })
    }
}